...
- **🧠 四级智能分类** URL 体检 → 规则匹配(300+ 规则) → 本地摘要二阶段 → AI兜底(DeepSeek)
- **📝 本地摘要** 网页摘要提取（description/H1/高频句，零网络），摘要关键词二次归类，命中即免 AI 调用
- **🌐 三重抓取引擎** Scrapling + requests + Firecrawl 兜底
- **📊 可视化审核** 颜色编码 + Excel 导出 + 内置对话框 + 失效链接筛选/一键删除
- **📁 两级分类** 14个大类 × N个小类（小类关键词精细化），标准 Netscape HTML 输出
...
3. 自动解析 → **URL 体检**（本地/失效分流）→ 规则分类（一级大类 + 二级小类）→ 本地摘要二阶段 → 可选 AI 补充 → 审核 → 生成分类后的 HTML
4. 用浏览器的「导入书签」功能把分类后的 HTML 导回浏览器

> 无需 AI Key 也能完成规则分类与 HTML 生成；网页抓取与 AI 分类可跳过。失效链接默认不写入导出 HTML（可在导出对话框中勾选包含），本地/内网书签默认包含。
