...
### v1.4 (配色升级：Indigo 现代科技)

- ✅ **全新色板（方案 A）**：主色 Blue `#3B82F6` → Indigo `#4F46E5` 家族，背景 `#F0F2F5` → `#F4F5FB` 微蓝浅灰，表头/选中/焦点环/复选框等全部同步为靛蓝系
- ✅ **更强层次**：卡片标题改靛蓝 900、表头靛蓝 50 浅染、统计卡 hover 靛蓝描边，摆脱"灰扑扑"
- ✅ **AI 点缀**：进度条靛蓝→紫渐变（`#4F46E5→#6366F1→#8B5CF6`），暗色主题更亮（`#6366F1→#818CF8→#A78BFA`）
- ✅ **对比度修复**：提示文字 `#94A3B8` → `#475569`（对比度 2.56 → 6.96），暗色 hint `#64748B` → `#94A3B8`（3.07 → 6.96），全部 WCAG 4.5+ 达标
- ✅ 浅色+暗色主题同步镜像，界面结构零改动；57 项测试全绿
