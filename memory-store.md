...
- **Indigo 配色（v2.1）**：换色时旧蓝系必须全量替换（#3B82F6/#2563EB/#1D4ED8/#1E40AF/#DBEAFE/#EFF6FF/#93C5FD/#60A5FA/#1E3A5F/#F0F2F5 残留扫描为 0）；暗色主题用亮一档的靛蓝（#6366F1 勾选/focus、#818CF8 文字），浅色用深一档（#4F46E5 主色、#4338CA hover）；hintText 对比度修复用 #475569（浅）/ #94A3B8（暗）。换色后必须跑 QSS 解析验证 + 旧色残留扫描 + 六组关键对比度核验。
- **Web 重构（v2.0）**：核心引擎全部纯 Python 无 GUI 依赖，UI 层是唯一 Qt 部分 → 重构面只在壳。三点关键经验：① SSE + TestClient 测试会挂死——httpx 关闭流时会 drain 无限响应，SSE 测试改为直接验证 `_json_snapshot()` 载荷 + 订阅/广播机制，不要用 `client.stream` 消费无限流；② `build_and_save` 对 0 条保留（全被排除）会校验失败返回 500——导出测试要保留至少 1 条，用「2 本地 + 1 正常」混合 fixture；③ `ProxyManager.test_connection` 直连 google.com 是真实网络——测试必须 mock。webapp 是单例全局（pipeline/config/secure_store 均为模块级），测试用 monkeypatch 换临时实例隔离；`modules/__init__.py` 曾 import 三个 Qt worker（fetch/classify/ai_worker），删桌面版时必须同步清理，否则 `import modules` 仍拉 PyQt6。
- **前端交互**：上传成功后必须自动调用 `startProcess()`（用户要的是"一键"，仅预览不处理是断的）；`reprocessBtn` 也应真正重跑。审核索引约定：前端与后端都用「未删除书签」的活跃索引（`filter(b => !b.user_deleted)[idx]` ↔ 后端 `[b for b in bookmarks if not b.user_deleted][index]`），保持一致。
